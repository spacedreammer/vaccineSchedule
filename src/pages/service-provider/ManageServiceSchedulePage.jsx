...
const ManageServiceSchedulePage = () => {
  ...
  const fetchMySchedule = async () => {
    ...
    setError(null);
    try {
      const toastId = "my-schedule-fetch-status";
      ...
      const res = await axios.get(`${API_BASE_URL}/provider/my-schedule`, {
        headers,
      });
      setMySchedule(res.data);
      toast.success("Your schedule loaded.", { toastId });
    } catch (err) {
      console.error(
      ...
      const toastId = "my-schedule-fetch-error";
      toast.error("Failed to load your schedule.", { toastId });
  ...
  const updateAppointmentStatus = async (appointmentId, status) => {
    const toastId = `schedule-update-${appointmentId}-${status}`;
    // This is typically done via "Mark Completed" page. This might be to cancel an assigned slot.
    if (
      !window.confirm(
        `Are you sure you want to change status to ${status} for this appointment?`
      )
    )
      return;
    setLoading(true);
    ...
    try {
      const headers = getAuthHeader();
      if (!headers.Authorization) {
        toast.error("Authentication required to update status.", { toastId });
        return;
      }
      await axios.put(
        `${API_BASE_URL}/provider/appointments/${appointmentId}/status`,
        { status },
        { headers }
      ); // Re-use /provider/appointments/{id}/status if it exists
      toast.success(`Appointment status updated to ${status}!`, { toastId });
  ...
  if (loading)
    return (
      <div className="p-6 text-center text-xl">Loading your schedule...</div>
    );
  if (error)
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white shadow-md rounded-lg mt-6">
      ...
      {mySchedule.length === 0 ? (
        <p className="text-gray-600">
          Your schedule is currently empty. No approved appointments assigned to
          you.
        </p>
      ) : (
            ...
            <tbody>
              {mySchedule.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4">
                    {item.preferred_time?.substring(0, 5) || "N/A"}
                  </td>{" "}
